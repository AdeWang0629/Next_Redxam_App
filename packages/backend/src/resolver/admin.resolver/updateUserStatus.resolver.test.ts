...
describe('user status resolver tests', () => {
  ...
  test('wrong admin token', async () => {
    ...
    const res = await updateUserStatus({ arg: { email: 'test@redxam.com', status: 'invited' } }, req);
  ...
  test('email not in waitlist', async () => {
    ...
    const res = await updateUserStatus({ arg: { email: 'nonexisted@redxam.com', status: 'invited' } }, req);
  ...
  test('user is already invited', async () => {
    ...
    const res = await updateUserStatus({ arg: { email: user.email, status: 'invited' } }, req);
